...
def payroll_employee(uid):
    ...
    try:
        ...
        work_history = []
    # v12.198 — Περίοδοι «χρήσης» στο Πρόγραμμα (read-only, από βάρδιες, strict ανά ξεν.)
    sched_first = sched_last = None; sched_periods = []
    try:
        from schedule import schedule_periods as _sp
        sched_periods = _sp(uid)
        if sched_periods:
            sched_first = min(p['start'] for p in sched_periods)
            sched_last  = max(p['end']   for p in sched_periods)
    except Exception:
        pass
...
def _inject_lock_badge():
    ...
    def lock_badge(u):
        ...
        if uid is None:
            return ''
        if uid in _ids():
            return Markup('<span title="Master \u03c0\u03c1\u03bf\u03c6\u03af\u03bb \u039b\u03bf\u03b3\u03b9\u03c3\u03c4\u03b7\u03c1\u03af\u03bf\u03c5 (Epsilon) \u2014 \u03ba\u03bb\u03b5\u03b9\u03b4\u03c9\u03bc\u03ad\u03bd\u03bf, \u03c0\u03b7\u03b3\u03ae \u03b1\u03bb\u03ae\u03b8\u03b5\u03b9\u03b1\u03c2" style="display:inline-flex;align-items:center;gap:3px;font-size:10px;font-weight:700;padding:1px 7px;border-radius:20px;background:#dbeafe;color:#1d4ed8;vertical-align:middle;white-space:nowrap;"><i class="ti ti-lock"></i> \u039f\u03c1\u03b9\u03c3\u03c4\u03b9\u03ba\u03bf\u03c0\u03bf\u03b9\u03b7\u03bc\u03ad\u03bd\u03bf</span>')
