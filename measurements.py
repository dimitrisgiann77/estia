...
ZNX_PARAMS = [
    ...
    ('clo2_dhw_out', 'ClO2 Αναχώρηση ΖΝΧ', 'ppm', 0.0, 2.0, None, _CLO2_HIGH.format(n='Αναχώρηση ΖΝΧ')),
    ('clo2_dhw_return', 'ClO2 Επιστροφή ΖΝΧ', 'ppm', 0.0, 2.0, None, _CLO2_HIGH.format(n='Επιστροφή ΖΝΧ')),
...
UNIFIED_WATER_PARAMS = [
    ...
    ('clo2_hot', 'ClO₂ (ζεστό)', 'ppm', 0.0, 2.0,
     None, 'ClO₂ ζεστού >2 ppm: μείωσε δοσομέτρηση.'),
]
...
def _entry_points():
    ...
    def _hot(nid):
        if nid not in _hs:
            ...
            _hs[nid] = _hotels_set(nn) if nn else set()
        return _hs[nid]
    def _node_ok(a):
        ...
        cur, seen = nid, 0
        while cur is not None and seen < 60:  # ο κόμβος Ή οποιοσδήποτε γονέας του
            if a.hotel_id in _hot(cur):
                return True
            cur = _pm.get(cur); seen += 1
        return False
